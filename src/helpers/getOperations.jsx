...
export const sumAmountTypes = async (type, operations) => {
    ...
    const filterOperType = await operations.filter(e => e.type.toString().toLowerCase() === type.toString().toLowerCase())
...
export const getOperation = async (type, operations) => {
    ...
    for( let key in groupByCategory){
        ...
        const sumCategory = () => {
            ...
            const ammount = groupByCategory[key].reduce((acc, currentAmount) => acc + currentAmount, 0)
...
export const getAmountCategories = (type, category) => {
    ...
    const shop = category.filter(e => Object.keys(e).toString().toLowerCase().split(' ')[0] === type.toString().toLowerCase())
